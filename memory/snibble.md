...
Snibble's claim & forfeit are CLIENT-SIDE sn_matches UPDATEs (matchActions.js claimMatchWin/forfeitMatch), not RPCs, so the existing round_submitted trigger never fired on them and the loser/surprise-winner got no push. Added `sn_matches.end_reason` ('claim'|'forfeit'); both client updates now set it. New `on_sn_match_ended` trigger (AFTER UPDATE in_progress→completed, WHEN end_reason IS NOT NULL) → pg_net 'game_finished'. Gated on end_reason so a normal last-round completion (already covered by round_submitted's "match complete!") does NOT double-push. Edge fn `game_finished` handler computes loser/winner purely from the record (winner_id + creator_id/opponent_id): claim → push loser, forfeit → push winner; same two messages as the other games. Migration `supabase/migrations/sn_matches_gameend_push.sql` applied via pooler; fn deployed. Verified both branches end-to-end with synthetic UUIDs (no sub → nothing delivered). No PWA/SW so no cache bump.
...
## 2026-07-02 — Server-side write guard + midnight auto-submit (c237, REFERENCE IMPL)
Closed the deliberate midnight daily-reveal cheat (past-day boards ungate at midnight per c92 → an open session could read revealed words and submit a padded score to yesterday). The c238 date-pin only fixed the *accidental* case; this is the airtight server guard.
- New `supabase/migrations/sn_daily_feeds_write_guard.sql` (applied to prod via pooler): two SECDEF RPCs — `sn_record_daily_feed` (STRICT today-only; the padding blocker) and `sn_finalize_daily_feed` (flips is_complete on an EXISTING row only, allowed today OR yesterday, can't touch words/score). Because words are banked incrementally pre-midnight, the finalize grace protects the honest cross-midnight finisher WITHOUT giving a padder anything to add. Dropped the direct `insert`/`update` RLS policies on `sn_daily_feeds` (read-own + delete-own stay) so the RPCs are the only writers — mirrors the sn_daily_puzzles lockdown.
- `src/hooks/useDailyState.js`: routes recordFeed → `sn_record_daily_feed`, markComplete → `sn_finalize_daily_feed`; added a 30s midnight-rollover watcher + `expireSession()` that finalizes the started day and sets `expired`. `GameView.jsx` shows a "🌙 a new day arrived — that daily is closed, what you fed is saved" note.
- Zero-downtime rollout: applied RPCs first (additive), pushed client (commit a1bc3a2), confirmed live bundle, THEN dropped policies. Guard verified via SQL boundary tests (past→reject, today→allow, finalize 2-days-ago→reject, yesterday→allow). Authed play-path not clicked through (SQ hub-login bounce, known limit).
...
**Snibble specifics:** no lobby row to hang a Map on, so the gate lives in `WaitingForOpponentPanel` (`MatchView.jsx`) as a single-opponent fetch — `opponentId = match.creator_id === user.id ? match.opponent_id : match.creator_id`. Simpler than Yahdle's fan-out. The fallback italic line had to change too: when the match is nudge-eligible on the clock but the opponent can't receive one, it now reads "You can claim the win after 7 days (N left)" instead of the old "You can nudge after 12 hours…", which would have been a lie. It deliberately says nothing about *why* — surfacing "they turned reminders off" would leak the opponent's settings. Commit `6714b0a`.

✅ **Resolved (c264, 2026-07-11):** split `sn_nudge` (validate-only, still `RETURNS uuid` target) + new `sn_mark_nudged(p_match_id)` (re-checks participant + in_progress, then stamps). `MatchView.jsx:handleNudge` calls `sn_mark_nudged` **only after** `postNudge` returns `delivered:true`; a failed stamp `console.warn`s, never throws. Migration `supabase/migrations/sn_nudge_cooldown_on_success.sql` — re-specified `SET search_path` (CREATE OR REPLACE clears it) + re-asserted the anon/public revoke. No SW. Verified at the data layer in a rolled-back txn: `sn_nudge` no longer writes `last_nudged_at`; `sn_mark_nudged` stamps for a participant, rejects a non-participant. Commit `70b7b30`. All four games now stamp only after confirmed delivery.
...
**Snibble SW:** none (no app-shell cache) → no bump; the heal runs in the HUB scope, so Snibble having no SW is irrelevant. Push fn: `supabase/functions/snibble-push-notification/index.ts`. Deployed + smoke-tested 200.
...
## 2026-08-16 — Rematch invites the actual opponent, reload-proof (c297)
c295-audit fix. `CompletedPanel.handleRematch` (`src/components/MatchView.jsx`) called `createMatch({ userId })` with no `invitedUserId` → every "Rematch" posted a PUBLIC open match any stranger could take, while the toast claimed "your opponent can rejoin." Now: derives `opponentId` (creator/opponent flip, same as forfeit/nudge), passes it as `invitedUserId` → private invite (`is_public=false`, 24h expiry) + real push via the existing `on_sn_match_invited` AFTER INSERT trigger (invites always pushed — the push lives in Postgres, not the client; `CreateMatchSheet`'s "bare" insert was never silent). Toast now "Rematch invite sent to {name}." Durable double-rematch gate: one select for an existing open invite (creator=me, invited=opponent, `created_at >= completed_at`) → toasts "Rematch invite already sent." + navigates to it, so re-opening the finished match can't spawn duplicates (reload-proof, deliberately not race-proof). Falls back to an open match with honest copy if no opponent id (admin-closed edge). Implemented by a Sonnet subagent; verified live in Chrome as Test vs Rae (fixture match via pooler-psql, session-injection login): invite row private+targeted, both toasts, gate navigates to existing, push log `sent:true` 200. Fixtures deleted after. Commit `ad9afdd`. Oublex/template cousin is c296 (separate codebase).
